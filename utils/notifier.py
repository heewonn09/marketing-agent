...
def notify_approval_pending(keywords: list, report_date: str, base_url: str = "") -> None:
    kw_str  = ", ".join(keywords)
    app_url = base_url or os.environ.get("CARDNEWS_BASE_URL", "")

    subject = f"[마케팅 에이전트] ⏳ 승인 대기: {kw_str}"
...
<div style="max-width:520px;margin:0 auto;background:white;border-radius:14px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,.1)">
  <div style="background:linear-gradient(135deg,#f59e0b,#d97706);padding:28px 32px;color:white">
    <div style="font-size:22px;font-weight:700;margin-bottom:4px">⏳ 콘텐츠 승인 대기</div>
    <div style="font-size:13px;opacity:.8">마케팅 자동화 에이전트</div>
  </div>
  <div style="padding:28px 32px">
    <p style="font-size:14px;color:#4a5568;margin:0 0 20px">
      콘텐츠 생성이 완료되었습니다.<br>
      대시보드에서 내용을 검토하고 승인하면 블로그·인스타그램에 자동으로 게시됩니다.
    </p>
    <table style="width:100%;border-collapse:collapse">
      <tr><td style="padding:8px 0;color:#718096;font-size:13px;width:80px">키워드</td>
          <td style="padding:8px 0;font-weight:600;color:#2d3748">{kw_str}</td></tr>
    ...
    {f'<div style="margin-top:24px"><a href="{app_url}" style="display:inline-block;padding:12px 28px;background:linear-gradient(135deg,#38a169,#2f855a);color:white;text-decoration:none;border-radius:8px;font-size:14px;font-weight:700">✓ 대시보드에서 승인하기</a></div>' if app_url else ''}
  </div>
  <div style="padding:16px 32px;background:#f7fafc;font-size:11px;color:#a0aec0;text-align:center">
    마케팅 자동화 에이전트 · 자동 발송 메일입니다
  </div>
    ...
    slack_text = (
        f"⏳ *콘텐츠 승인 대기*\n"
        f"키워드: `{kw_str}`  |  날짜: {report_date}\n"
        f"콘텐츠 생성 완료 — 검토 후 승인하면 블로그·인스타에 게시됩니다."
        + (f"\n<{app_url}|✓ 대시보드에서 승인하기>" if app_url else "")
    )
