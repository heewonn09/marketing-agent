...
### 타이포그래피 마킹
- 포스트 제목: 나눔스퀘어 32pt Bold → 별도 title 필드에 작성
- 대제목(H2): [QUOTE]소제목 텍스트[/QUOTE] 형식으로 마킹 (네이버 에디터 인용구 스타일 적용용)
- 본문: 나눔스퀘어 15~16pt, 행간 180% → 별도 설정 불필요, 텍스트만 작성

### 가독성 규칙
1. 3초 스캔 법칙: 각 문단의 핵심 수치·키워드(예: "수익 200%", "시간 50% 단축")는 **반드시 __볼드+하이라이트__ 처리** → `**[HL]텍스트[/HL]**` 형식으로 마킹
2. 한 문단 최대 3~4줄 (모바일 기준). 문단 사이는 빈 줄 하나로 구분
3. 나열형 데이터·사례는 `•` 불릿으로 요약박스 변환:
   ```
   • 항목1: 수치/내용
   • 항목2: 수치/내용
   • 항목3: 수치/내용
...
### 이미지 자리 표시
각 H2 섹션이 끝나는 지점(첫 번째·두 번째·네 번째 섹션 필수)에 `[IMAGE: 구체적 이미지 설명(3D 일러스트/인포그래픽/사진 톤 명시)]` 태그 삽입.
- 섹션 1(AI·자동화 관련): [IMAGE: 노트북 앞에서 AI 도구로 콘텐츠 캘린더를 생성하며 미소 짓는 프리랜서 3D 일러스트, 밝고 따뜻한 색감]
- 섹션 2(매출·수익 관련): [IMAGE: 카페 내부 전경과 매출 그래프 우상향 인포그래픽, 따뜻한 감성 디자인]
- 섹션 4(이미지·컬러·퍼스널 관련): [IMAGE: 쿨톤/웜톤 팔레트 배경 위 전문적 프로필 실루엣 세련된 시각 자료]
- 주제와 맞지 않는 섹션은 해당 섹션 내용에 어울리는 고화질 이미지 가이드로 대체

### 박스형 CTA (글 마지막 필수)
본문 마지막에 아래 형식의 CTA 박스를 반드시 추가:
```
---
[CTA_BOX]
💡 **함께 읽으면 도움이 되는 추천 칼럼**

• [{keyword} 관련 추천 글 제목 1 (실제 인사이트 기반으로 작성)](https://blog.naver.com/heewonnn09)
• [{keyword} 관련 추천 글 제목 2 (실제 인사이트 기반으로 작성)](https://blog.naver.com/heewonnn09)

📌 이 글이 도움됐다면 **이웃추가** 후 매주 업데이트되는 마케팅 인사이트를 받아보세요!
[/CTA_BOX]
---
```
...
{{
  "naver_blog": {{
    ...
    "body": "블로그 본문 (2500자 이상, 위 포스트 가이드·디자인 포맷 규칙 모두 준수. [QUOTE]소제목[/QUOTE] 형식으로 H2 마킹, 핵심 수치에 **[HL]텍스트[/HL]** 적용, 각 섹션 끝에 [IMAGE:...] 태그 삽입, 나열 데이터는 • 불릿 변환, 마지막에 [CTA_BOX]...[/CTA_BOX] 박스 삽입. '{keyword}'를 본문에 10~15회 자연스럽게 포함(키워드 밀도 0.4~0.6%), 연관 키워드({top_keywords}) 각 섹션에 분산 배치)",
